...
# ========================================
# CAUSAL-PROTECTED ADVERSARIAL DEFENSE
# Integrating Doubly Robust Estimation to Filter Bad LLM Advice
# ========================================
...
# ========================================
# LIGHTWEIGHT ONLINE CAUSAL FILTER
# ========================================

class OnlineCausalFilter:
    """
    Real-time causal filter to detect and reject harmful LLM advice
    
    Uses lightweight version of Doubly Robust estimation during training
    to predict whether LLM advice will help or hurt.
    """
    ...
    def __init__(self, warmup_steps=500, update_frequency=100):
        ...
        # Models (lightweight for online use)
        self.propensity_model = None
        self.outcome_model_with_llm = None
        self.outcome_model_without_llm = None
        ...
        self.feature_cols = [
            'health_ratio', 'level_norm', 'depth_norm', 
            'reward_lag1', 'reward_ma5', 'episode_progress'
        ]
        
        print("🛡️  Causal Filter initialized")
        print(f"   Warmup period: {warmup_steps} steps")
        print(f"   Update frequency: {update_frequency} steps")
    
    def collect_data(self, obs, processed_obs, action, reward, shaped_reward, 
                     llm_advice_given, episode_step, episode_length):
        """Collect data point for causal analysis"""
        
        # Extract features
        stats = obs.get('blstats', np.zeros(26)) if isinstance(obs, dict) else np.zeros(26)
        
        hp = float(stats[10]) if len(stats) > 10 else 0
        max_hp = float(stats[11]) if len(stats) > 11 else 1
        health_ratio = hp / max_hp if max_hp > 0 else 0.5
        
        level = int(stats[18]) if len(stats) > 18 else 1
        depth = int(stats[12]) if len(stats) > 12 else 1
        
        # Calculate lag features
        reward_lag1 = self.step_history[-1]['shaped_reward'] if self.step_history else 0
        reward_ma5 = np.mean([s['shaped_reward'] for s in self.step_history[-5:]]) if len(self.step_history) >= 5 else 0
        
        episode_progress = episode_step / episode_length if episode_length > 0 else 0
        ...
        data_point = {
            ...
            'llm_advice_given': llm_advice_given,
            'action': action
        }
    ...
    def should_update_models(self):
        """Check if we should update causal models"""
        return (self.current_step >= self.warmup_steps and 
                self.current_step % self.update_frequency == 0)
    
    def update_causal_models(self):
        """Update causal models from collected data"""
        
        if len(self.step_history) < self.warmup_steps:
            return False
        
        print(f"\n🔄 Updating causal models (step {self.current_step})...")
        
        try:
            # Convert to DataFrame
            df = pd.DataFrame(self.step_history[-2000:])  # Use last 2000 steps
            
            # Prepare features
            X = df[self.feature_cols].values
            T = df['llm_advice_given'].values
            Y = df['shaped_reward'].values
            ...
            # 2. Train outcome models
            with_llm_mask = T == 1
            without_llm_mask = T == 0
            
            if np.sum(with_llm_mask) > 50:
                ...
                self.outcome_model_with_llm.fit(X_scaled[with_llm_mask], Y[with_llm_mask])
            ...
            if np.sum(without_llm_mask) > 50:
                ...
                self.outcome_model_without_llm.fit(X_scaled[without_llm_mask], Y[without_llm_mask])
            
            print(f"   ✅ Models updated with {len(df)} data points")
            return True
    ...
    def should_accept_llm_advice(self, obs, processed_obs, episode_step, episode_length):
        """
        Decide whether to accept or reject LLM advice using causal prediction
        
        Returns:
            (should_accept: bool, predicted_effect: float, confidence: float)
        """
        
        # Before warmup, always accept (no models yet)
        if self.current_step < self.warmup_steps:
            self.acceptances += 1
            return True, 0.0, 0.0
        
        # If models not trained, accept
        if (self.propensity_model is None or 
            self.outcome_model_with_llm is None or 
            self.outcome_model_without_llm is None):
            self.acceptances += 1
            return True, 0.0, 0.0
        ...
        try:
            ...
            # Predict outcomes with and without LLM
            ...
            # Get propensity score for confidence
            propensity = self.propensity_model.predict_proba(features_scaled)[0, 1]
            confidence = abs(predicted_effect)
            
            # Decision threshold
            THRESHOLD = -0.01  # Reject if predicted effect < -0.01
            ...
            if should_accept:
                ...
                self.rejections += 1
                print(f"   🚫 Causal filter REJECTED LLM advice (predicted effect: {predicted_effect:.4f})")
            ...
            print(f"   ⚠️ Causal prediction error: {e}")
            self.acceptances += 1
            return True, 0.0, 0.0
    
    def get_stats(self):
        """Get filter statistics"""
        rejection_rate = self.rejections / max(1, self.predictions_total)
        ...
        return {
            ...
            'data_collected': len(self.step_history)
        }
    
    def reset_episode(self):
        """Reset per-episode state"""
        # Keep step history for learning, but could add episode markers
        pass


# ========================================
# CAUSAL-PROTECTED ADVERSARIAL AGENT
# ========================================

class CausalProtectedAdversarialAgent(AdversarialLLMEnhancedAgent):
    """
    Agent with causal filtering to protect against adversarial attacks
    """
    
    def __init__(self, *args, use_causal_filter=True, **kwargs):
        super().__init__(*args, **kwargs)
        
        self.use_causal_filter = use_causal_filter
        
        if use_causal_filter:
            self.causal_filter = OnlineCausalFilter(
                warmup_steps=500,
                update_frequency=100
            )
            print(f"🛡️  Causal protection: ENABLED")
        else:
            self.causal_filter = None
            print(f"⚠️  Causal protection: DISABLED")
        
        # Tracking
        self.episode_step = 0
        self.episode_length_estimate = 100  # Will be updated
        self.llm_advice_rejected_count = 0
        self.llm_advice_accepted_count = 0
    
    async def select_action(self, tensor_obs, processed_obs, raw_obs, 
                           reset_hidden=False, performance_metrics=None):
        """Action selection with causal filtering"""
        
        # Get LLM hints if enabled
        llm_hints_proposed = None
        llm_advice_given = False
        
        if self.llm_advisor and performance_metrics:
            if self.llm_advisor.should_call_llm(performance_metrics):
                print(f"\n🤖 LLM providing strategic advice...")
                
                # Get LLM advice (potentially attacked)
                llm_hints_proposed = await self.llm_advisor.get_strategic_advice(
                    raw_obs, processed_obs, performance_metrics
                )
                
                # 🛡️ CAUSAL FILTERING STEP
                if self.use_causal_filter and self.causal_filter:
                    should_accept, predicted_effect, confidence = \
                        self.causal_filter.should_accept_llm_advice(
                            raw_obs, processed_obs, 
                            self.episode_step, self.episode_length_estimate
                        )
                    
                    if should_accept:
                        self.current_llm_hints = llm_hints_proposed
                        llm_advice_given = True
                        self.llm_call_count += 1
                        self.llm_advice_accepted_count += 1
                        print(f"   ✅ Causal filter ACCEPTED advice (predicted effect: {predicted_effect:+.4f})")
                    else:
                        self.current_llm_hints = None  # REJECT - don't use LLM hints
                        llm_advice_given = False
                        self.llm_advice_rejected_count += 1
                        print(f"   🚫 Causal filter REJECTED advice (would harm performance)")
                else:
                    # No filtering - always accept
                    self.current_llm_hints = llm_hints_proposed
                    llm_advice_given = True
                    self.llm_call_count += 1
        ...
        with torch.no_grad():
            ...
            return action.item(), log_prob.item(), value.item(), llm_advice_given
    
    async def train(self, env, num_episodes=100, update_freq=1024, print_freq=10):
        """Training with causal filtering"""
        step_count = 0
        global_step = 0
        ...
        if self.actor.use_llm:
            print(f"   Causal Protection: {'ENABLED 🛡️' if self.use_causal_filter else 'DISABLED ⚠️'}")
        print()
        ...
        for episode in range(num_episodes):
            ...
            if self.causal_filter:
                self.causal_filter.reset_episode()
            ...
            while True:
                ...
                # Action selection with causal filtering
                action, log_prob, value, llm_advice_given = await self.select_action(
                    tensor_obs, processed_obs, raw_obs, reset_hidden, performance_metrics
                )
                reset_hidden = False
                ...
                # Collect data for causal filter
                if self.causal_filter:
                    self.causal_filter.collect_data(
                        raw_obs, processed_obs, action, reward, shaped_reward,
                        llm_advice_given, self.episode_step, 
                        self.episode_length_estimate
                    )
                    
                    # Update models periodically
                    if self.causal_filter.should_update_models():
                        self.causal_filter.update_causal_models()
            ...
            # Update episode length estimate
            self.episode_length_estimate = int(0.9 * self.episode_length_estimate + 0.1 * episode_length)
            ...
            if episode % print_freq == 0:
                ...
                causal_info = ""
                if self.causal_filter:
                    stats = self.causal_filter.get_stats()
                    causal_info = f", Rejected: {stats['rejections']}/{stats['total_predictions']}"
                ...
                print(f"📈 Episode {episode}: "
                      f"Avg Reward: {avg_reward:.3f}, "
                      f"Avg Length: {avg_length:.1f}"
                      f"{llm_info}{causal_info}")
        
        # Final stats
        if self.use_causal_filter and self.causal_filter:
            stats = self.causal_filter.get_stats()
            print(f"\n🛡️  Causal Filter Statistics:")
            print(f"   Total Predictions: {stats['total_predictions']}")
            print(f"   Accepted: {stats['acceptances']}")
            print(f"   Rejected: {stats['rejections']}")
            print(f"   Rejection Rate: {stats['rejection_rate']:.1%}")
        
        if self.actor.use_llm:
            print(f"\n🤖 LLM Statistics:")
            print(f"   Total Advice Requests: {self.llm_call_count}")
            print(f"   Accepted by Filter: {self.llm_advice_accepted_count}")
            print(f"   Rejected by Filter: {self.llm_advice_rejected_count}")
        
        return list(self.episode_rewards), list(self.shaped_rewards)


# ========================================
# MONITORED CAUSAL-PROTECTED AGENT
# ========================================

class MonitoredCausalProtectedAgent(CausalProtectedAdversarialAgent):
    """Agent with both monitoring and causal protection"""
    ...
    async def train(self, env, num_episodes=100, update_freq=1024, print_freq=10):
        """Training with monitoring and causal protection"""
        step_count = 0
        
        print(f"🚀 Training for {num_episodes} episodes with monitoring & causal protection")
        ...
        for episode in range(num_episodes):
            ...
            while True:
                ...
                # Track LLM calls
                old_llm_count = self.llm_call_count
                ...
                # Check if LLM was called
                new_llm_count = self.llm_call_count
                if new_llm_count > old_llm_count:
                    self.llm_calls_this_episode += 1
                    if self.llm_advisor:
                        self.monitor.log_attack_event(episode, self.llm_advisor.attacker.attack_type.value)
            ...
            if episode % print_freq == 0:
                ...
                if self.causal_filter:
                    stats = self.causal_filter.get_stats()
                    causal_info = f", Rejected: {stats['rejections']}"
                
                print(f"📈 Episode {episode}: Avg Reward: {avg_reward:.3f}, Avg Length: {avg_length:.1f}{causal_info}")
        
        # Final stats
        if self.causal_filter:
            stats = self.causal_filter.get_stats()
            print(f"\n🛡️  Causal Filter Final Stats:")
            print(f"   Rejection Rate: {stats['rejection_rate']:.1%}")
            print(f"   Total Rejections: {stats['rejections']}")
        
        return list(self.episode_rewards), list(self.shaped_rewards)


# ========================================
# COMPREHENSIVE CAUSAL DEFENSE EXPERIMENTS
# ========================================

async def run_causal_defense_experiments():
    """
    Run comprehensive experiments comparing:
    1. Baseline (no LLM)
    2. LLM with attacks (no protection)
    3. LLM with attacks + Causal filtering (protected)
    """
    
    print("="*80)
    print("🛡️  CAUSAL DEFENSE AGAINST ADVERSARIAL ATTACKS")
    print("="*80)
    ...
    for config in attack_configs:
        ...
        print(f"   Attack: {config['attack_type'].value}")
        print(f"   Causal Protection: {config['use_causal']}")
        print(f"{'='*80}\n")
        ...
        # Create agent
        agent = MonitoredCausalProtectedAgent(
            action_dim=env.action_space.n,
            enable_llm=config['use_llm'],
            llm_guidance_weight=0.05,
            attacker=attacker,
            use_causal_filter=config['use_causal'],
            monitor=monitor
        )
        ...
        # Store results
        monitors[config['name']] = monitor
        monitor.save_to_file(f"{config['name'].replace(' ', '_').replace('(', '').replace(')', '').lower()}_monitor.json")
        
        stats = monitor.get_summary_stats()
        print(f"\n✅ {config['name']} Complete!")
        print(f"   Final Avg: {stats['final_avg']:.3f}")
        print(f"   Mean ± Std: {stats['mean_reward']:.3f} ± {stats['std_reward']:.3f}")
        print(f"   Time: {train_time:.1f}s")
    
    env.close()
    
    # Set baseline for degradation calculation
    baseline_name = 'Baseline (No LLM)'
    if baseline_name in monitors:
        baseline_rewards = monitors[baseline_name].moving_avg_rewards
        for name, monitor in monitors.items():
    ...
    # Create visualizations
    print("\n" + "="*80)
    print("📊 GENERATING CAUSAL DEFENSE VISUALIZATIONS")
    print("="*80)
    ...
    viz.plot_degradation_during_training(
        monitors,
        save_path='causal_defense_degradation.png'
    )
    
    # Heatmap
    viz.plot_attack_impact_heatmap(
        monitors,
        save_path='causal_defense_heatmap.png'
    )
    
    # Create causal-specific comparison plot
    plot_causal_protection_comparison(monitors)
    
    # Print comprehensive summary
    print_causal_defense_summary(monitors)
    
    # Generate LaTeX table
    generate_causal_defense_latex(monitors)
    
    print("\n✅ All causal defense experiments complete!")
    print("\n📁 Generated files:")
    print("   - causal_defense_degradation.png")
    print("   - causal_defense_heatmap.png")
    print("   - causal_protection_comparison.png")
    print("   - causal_defense_results.tex")
    
    return monitors


# ========================================
# CAUSAL-SPECIFIC VISUALIZATIONS
# ========================================

def plot_causal_protection_comparison(monitors_dict):
    """Create focused comparison of causal protection effectiveness"""
    
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Causal Protection Against Adversarial Attacks', 
                 fontsize=16, fontweight='bold')
    
    # Extract attack pairs (unprotected vs protected)
    attack_pairs = []
    for name in monitors_dict.keys():
        if 'Unprotected' in name:
            attack_type = name.split('(')[0].strip()
            protected_name = f"{attack_type} (Causal Protected)"
            if protected_name in monitors_dict:
                attack_pairs.append((name, protected_name, attack_type))
    
    # ========================================
    # PLOT 1: Reward curves comparison
    # ========================================
    ax1 = axes[0, 0]
    
    colors_unprotected = ['red', 'orange']
    colors_protected = ['green', 'blue']
    
    # Plot baseline
    if 'Baseline (No LLM)' in monitors_dict:
        baseline = monitors_dict['Baseline (No LLM)']
        episodes = range(len(baseline.moving_avg_rewards))
        ax1.plot(episodes, baseline.moving_avg_rewards, 
                label='Baseline (No LLM)', linewidth=2.5, 
                color='black', alpha=0.8, linestyle='--')
    
    # Plot attack pairs
    for idx, (unprotected_name, protected_name, attack_type) in enumerate(attack_pairs):
        if unprotected_name in monitors_dict:
            mon = monitors_dict[unprotected_name]
            episodes = range(len(mon.moving_avg_rewards))
            ax1.plot(episodes, mon.moving_avg_rewards,
                    label=f'{attack_type} (Unprotected)',
                    linewidth=2, color=colors_unprotected[idx], alpha=0.7)
        
        if protected_name in monitors_dict:
            mon = monitors_dict[protected_name]
            episodes = range(len(mon.moving_avg_rewards))
            ax1.plot(episodes, mon.moving_avg_rewards,
                    label=f'{attack_type} (Protected 🛡️)',
                    linewidth=2.5, color=colors_protected[idx], alpha=0.9)
    
    ax1.set_xlabel('Episode', fontsize=12)
    ax1.set_ylabel('Moving Average Reward', fontsize=12)
    ax1.set_title('Performance: Unprotected vs Causal-Protected', fontsize=13, fontweight='bold')
    ax1.legend(loc='best', fontsize=9)
    ax1.grid(True, alpha=0.3)
    ...
    if 'Baseline (No LLM)' in monitors_dict:
        stats = monitors_dict['Baseline (No LLM)'].get_summary_stats()
        comparison_data.append(stats['final_avg'])
        labels.append('Baseline')
        colors.append('gray')
    
    for unprotected_name, protected_name, attack_type in attack_pairs:
        if unprotected_name in monitors_dict:
            stats = monitors_dict[unprotected_name].get_summary_stats()
            comparison_data.append(stats['final_avg'])
            labels.append(f'{attack_type}\n(Unprotected)')
            colors.append('indianred')
        
        if protected_name in monitors_dict:
            stats = monitors_dict[protected_name].get_summary_stats()
            comparison_data.append(stats['final_avg'])
            labels.append(f'{attack_type}\n(Protected)')
            colors.append('mediumseagreen')
    
    bars = ax2.bar(range(len(comparison_data)), comparison_data, 
                   color=colors, alpha=0.7, edgecolor='black', linewidth=1.5)
    ax2.set_xticks(range(len(labels)))
    ax2.set_xticklabels(labels, rotation=0, ha='center', fontsize=9)
    ax2.set_ylabel('Final Average Reward', fontsize=12)
    ax2.set_title('Final Performance Comparison', fontsize=13, fontweight='bold')
    ax2.grid(True, alpha=0.3, axis='y')
    
    # Add value labels
    for i, (bar, val) in enumerate(zip(bars, comparison_data)):
        ax2.text(bar.get_x() + bar.get_width()/2, val,
                f'{val:.2f}',
                ha='center', va='bottom', fontsize=9, fontweight='bold')
    
    # ========================================
    # PLOT 3: Protection effectiveness (% recovery)
    # ========================================
    ax3 = axes[1, 0]
    
    baseline_perf = monitors_dict['Baseline (No LLM)'].get_summary_stats()['final_avg'] if 'Baseline (No LLM)' in monitors_dict else 0
    
    recovery_rates = []
    attack_labels = []
    
    for unprotected_name, protected_name, attack_type in attack_pairs:
        if unprotected_name in monitors_dict and protected_name in monitors_dict:
            unprotected_stats = monitors_dict[unprotected_name].get_summary_stats()
            protected_stats = monitors_dict[protected_name].get_summary_stats()
            
            unprotected_perf = unprotected_stats['final_avg']
            protected_perf = protected_stats['final_avg']
            
            # Calculate how much of the performance loss was recovered
            if baseline_perf > 0:
                performance_loss = baseline_perf - unprotected_perf
                performance_recovery = protected_perf - unprotected_perf
    ...
    bars = ax3.barh(attack_labels, recovery_rates, 
                    color='mediumseagreen', alpha=0.7, edgecolor='black', linewidth=1.5)
    ax3.axvline(x=100, color='green', linestyle='--', linewidth=2, alpha=0.5, label='100% Recovery')
    ax3.set_xlabel('Performance Recovery (%)', fontsize=12)
    ax3.set_title('Causal Protection Effectiveness', fontsize=13, fontweight='bold')
    ...
    # ========================================
    # PLOT 4: Degradation over time comparison
    # ========================================
    ax4 = axes[1, 1]
    ...
    for idx, (unprotected_name, protected_name, attack_type) in enumerate(attack_pairs):
        if unprotected_name in monitors_dict:
            mon = monitors_dict[unprotected_name]
            if mon.degradation_over_time:
                episodes = range(len(mon.degradation_over_time))
                ax4.plot(episodes, mon.degradation_over_time,
                        label=f'{attack_type} (Unprotected)',
                        linewidth=2, color=colors_unprotected[idx], alpha=0.7,
                        linestyle='--')
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f'causal_protection_comparison_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"📊 Causal protection comparison saved: {filename}")
    plt.close()


def print_causal_defense_summary(monitors_dict):
    """Print comprehensive summary of causal defense results"""
    
    print("\n" + "="*100)
    print("📋 CAUSAL DEFENSE COMPREHENSIVE SUMMARY")
    print("="*100)
    
    baseline_name = 'Baseline (No LLM)'
    baseline_perf = monitors_dict[baseline_name].get_summary_stats()['final_avg'] if baseline_name in monitors_dict else 0
    
    print(f"\n{'Experiment':<40} {'Final Avg':<12} {'Degradation':<15} {'Recovery':<15} {'Mean±Std':<20}")
    print("-"*100)
    
    # Print baseline
    if baseline_name in monitors_dict:
        stats = monitors_dict[baseline_name].get_summary_stats()
        print(f"{baseline_name:<40} {stats['final_avg']:<12.3f} {'-':<15} {'-':<15} {stats['mean_reward']:.3f}±{stats['std_reward']:.3f}")
        print("-"*100)
    
    # Extract attack pairs
    attack_pairs = []
    for name in monitors_dict.keys():
        if 'Unprotected' in name:
            attack_type = name.split('(')[0].strip()
    ...
    for unprotected_name, protected_name, attack_type in attack_pairs:
        ...
        if unprotected_name in monitors_dict:
            stats = monitors_dict[unprotected_name].get_summary_stats()
            degradation = ((baseline_perf - stats['final_avg']) / abs(baseline_perf)) * 100 if baseline_perf != 0 else 0
            print(f"  {'Unprotected':<38} {stats['final_avg']:<12.3f} {degradation:<14.1f}% {'-':<15} {stats['mean_reward']:.3f}±{stats['std_reward']:.3f}")
        ...
        if protected_name in monitors_dict:
            ...
            if unprotected_name in monitors_dict:
                unprotected_stats = monitors_dict[unprotected_name].get_summary_stats()
                unprotected_perf = unprotected_stats['final_avg']
                protected_perf = stats['final_avg']
                
                performance_loss = baseline_perf - unprotected_perf
                performance_recovery = protected_perf - unprotected_perf
                recovery_rate = (performance_recovery / performance_loss * 100) if performance_loss > 0 else 0
                recovery_str = f"{recovery_rate:.1f}%"
            else:
                recovery_str = "-"
            
            print(f"  {'Protected (Causal)':<38} {stats['final_avg']:<12.3f} {degradation:<14.1f}% {recovery_str:<15} {stats['mean_reward']:.3f}±{stats['std_reward']:.3f}")
    
    print("\n" + "="*100)
    
    # Calculate overall statistics
    print(f"\n📊 OVERALL CAUSAL PROTECTION STATISTICS:")
    ...
    for unprotected_name, protected_name, attack_type in attack_pairs:
        if unprotected_name in monitors_dict and protected_name in monitors_dict:
            ...
            all_recoveries.append(recovery_rate)
    
    if all_recoveries:
        print(f"   Average Recovery Rate: {np.mean(all_recoveries):.1f}%")
        print(f"   Best Recovery: {np.max(all_recoveries):.1f}%")
        print(f"   Worst Recovery: {np.min(all_recoveries):.1f}%")
    
    print("="*100 + "\n")


def generate_causal_defense_latex(monitors_dict, output_file='causal_defense_results.tex'):
    """Generate LaTeX table for research paper"""
...
"""
    ...
    if baseline_name in monitors_dict:
        stats = monitors_dict[baseline_name].get_summary_stats()
        latex += f"Baseline & None & {stats['final_avg']:.2f} & - & - \\\\\n"
        latex += r"\midrule" + "\n"
    ...
    for unprotected_name, protected_name, attack_type in attack_pairs:
        ...
        if unprotected_name in monitors_dict:
            stats = monitors_dict[unprotected_name].get_summary_stats()
            degradation = ((baseline_perf - stats['final_avg']) / abs(baseline_perf)) * 100 if baseline_perf != 0 else 0
            latex += f"{attack_type} & None & {stats['final_avg']:.2f} & {degradation:.1f} & - \\\\\n"
        ...
        if protected_name in monitors_dict:
            ...
            latex += f"{attack_type} & Causal & {stats['final_avg']:.2f} & {degradation:.1f} & {recovery_rate:.1f} \\\\\n"
...
# ========================================
# QUICK CAUSAL DEFENSE TEST
# ========================================

async def quick_causal_defense_test():
    """Quick test to verify causal protection works"""
    
    print("="*80)
    print("⚡ QUICK CAUSAL DEFENSE TEST")
    print("="*80)
    ...
    for config in configs:
        ...
        agent = CausalProtectedAdversarialAgent(
            action_dim=env.action_space.n,
            enable_llm=config['use_llm'],
            llm_guidance_weight=0.05,
            attacker=attacker,
        ...
        rewards, _ = await agent.train(env, num_episodes=num_episodes, update_freq=256, print_freq=2)
        ...
        print(f"✅ {config['name']}: Mean = {results[config['name']]['mean']:.3f}")
    ...
    unprotected = results['Attack Unprotected']['mean']
    protected = results['Attack Protected']['mean']
    
    degradation_unprotected = ((baseline - unprotected) / abs(baseline)) * 100 if baseline != 0 else 0
    degradation_protected = ((baseline - protected) / abs(baseline)) * 100 if baseline != 0 else 0
    
    recovery = ((protected - unprotected) / (baseline - unprotected)) * 100 if (baseline - unprotected) != 0 else 0
    
    print(f"Baseline:            {baseline:.3f}")
    print(f"Attack Unprotected:  {unprotected:.3f} (degradation: {degradation_unprotected:.1f}%)")
    print(f"Attack Protected:    {protected:.3f} (degradation: {degradation_protected:.1f}%)")
    print(f"Recovery Rate:       {recovery:.1f}%")
    
    if recovery > 50:
        print(f"\n✅ Causal protection is WORKING! ({recovery:.1f}% recovery)")
    else:
        print(f"\n⚠️ Causal protection needs tuning ({recovery:.1f}% recovery)")
...
if __name__ == "__main__":
    ...
    print("\n🛡️  CAUSAL DEFENSE AGAINST ADVERSARIAL ATTACKS")
    print("="*80)
    print("\nAvailable modes:")
    print("  1. full       - Run full causal defense experiments")
    print("  2. quick      - Quick test (10 episodes)")
    print("  3. monitored  - Full experiments with detailed monitoring")
    print("\n" + "="*80)
    ...
    if mode == "full":
        print("\n🚀 Running full causal defense experiments...")
        monitors = asyncio.run(run_causal_defense_experiments())
        
    elif mode == "quick":
        print("\n⚡ Running quick causal defense test...")
        results = asyncio.run(quick_causal_defense_test())
        
    elif mode == "monitored":
        print("\n📊 Running monitored causal defense experiments...")
        monitors = asyncio.run(run_causal_defense_experiments())
        compare_attack_effectiveness(monitors)
        
    else:
        print(f"❌ Unknown mode: {mode}")
        print("   Use: full, quick, or monitored")
        sys.exit(1)
