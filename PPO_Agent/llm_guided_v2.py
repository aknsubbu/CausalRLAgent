...
class NetHackRewardShaper:
    """Enhanced reward shaping with better balance and exploration incentives"""
    ...
    def __init__(self):
        ...
        self.episode_combat_count=0
        self.step_count = 0
        
        # FIX: ADD THESE MISSING ATTRIBUTES
        self.last_10_actions = deque(maxlen=10)
        self.stuck_detection_threshold = 7
        
        # IMPROVED: Better balanced reward weights
        self.exploration_reward = 0.5  # INCREASED for more exploration
        self.revisit_penalty = -0.02
        self.health_loss_penalty = -0.15
        self.health_gain_reward = 0.7
        self.level_reward = 100.0  # INCREASED - huge motivation
        self.experience_reward = 1.0 # INCREASED
        self.death_penalty = -20.0  # INCREASED penalty
        self.stuck_penalty = -0.1
        self.item_reward = 0.5
        self.combat_reward = 10.0
        self.repetition_penalty = -0.05
        self.descent_reward = 10.0  # INCREASED
        self.gold_reward = 1.0
        self.move_efficiency_bonus = 0.03

        self.survival_bonus=0.01
        self.movement_bonus = 0.02  # Reward for moving
        self.stuck_penalty = -0.3  # Stronger stuck penalty
        
    def shape_reward(self, obs, raw_reward, done, info, action=None):
        """Improved reward shaping with better signal-to-noise ratio"""
        ...
        if len(current_stats) < 20:
            return shaped_reward
        
        if self.previous_stats is not None:
            # Health changes (index 10, 11)
            health_diff = current_stats[10] - self.previous_stats[10]
            if health_diff > 0:
                bonus = health_diff * self.health_gain_reward
                shaped_reward += bonus
                print(f"💚 HEALED! +{health_diff} HP, Bonus: +{bonus:.2f}")
            elif health_diff < 0:
                penalty = health_diff * self.health_loss_penalty
                shaped_reward += penalty
                print(f"💔 DAMAGED! {health_diff} HP, Penalty: {penalty:.2f}")
            
            # Level up (index 18) - CRITICAL for learning
            level_diff = current_stats[18] - self.previous_stats[18]
            if level_diff > 0:
                shaped_reward += self.level_reward
                print(f"🎉 LEVEL UP! Bonus: +{self.level_reward}")
            
            # Experience gain (index 19)
            exp_diff = current_stats[19] - self.previous_stats[19]
            if exp_diff > 0:
                shaped_reward += exp_diff * self.experience_reward
                # self.episode_combat_count += 1
                
                # Extra reward for significant exp (monster kill)
                if exp_diff > 10:
                    shaped_reward += self.combat_reward
                    print(f"⚔️ Monster killed! XP: +{exp_diff}, Bonus: +{self.combat_reward}")
            
            # Dungeon depth progression (index 12)
            depth_diff = current_stats[12] - self.previous_stats[12]
            if depth_diff > 0:
                shaped_reward += self.descent_reward
                print(f"⬇️ Descended! Bonus: +{self.descent_reward}")
            
            # Gold collection (index 13)
            gold_diff = current_stats[13] - self.previous_stats[13]
            if gold_diff > 0:
                gold_bonus = gold_diff * self.gold_reward
                shaped_reward += gold_bonus
                print(f"💰 GOLD! +{gold_diff}, Bonus: +{gold_bonus:.2f}")
        ...
        if current_pos not in self.visited_positions:
            ...
            # Small penalty for revisiting (encourages new exploration)
            shaped_reward += self.revisit_penalty
        
        # Movement efficiency: reward for moving (not stuck)
        if current_pos != self.last_position:
            shaped_reward += self.move_efficiency_bonus
            self.consecutive_stuck_steps = 0
        else:
            self.consecutive_stuck_steps += 1
            
            # Progressive stuck penalty
            if self.consecutive_stuck_steps > 5:
                stuck_multiplier = min(self.consecutive_stuck_steps / 10.0, 3.0)
                shaped_reward += self.stuck_penalty * stuck_multiplier
        
        # Action repetition penalty with tolerance
        if action == self.last_action:
            self.action_repetition_count += 1
            # Only penalize after 5 consecutive repeats
            if self.action_repetition_count > 5:
                repeat_multiplier = min((self.action_repetition_count - 5) / 5.0, 2.0)
                shaped_reward += self.repetition_penalty * repeat_multiplier
        else:
            self.action_repetition_count = 0
        ...
        # CRITICAL: Detect stagnation and apply MASSIVE penalty
        if action is not None:
            self.last_10_actions.append(action)
            ...
            if len(self.last_10_actions) == 10:
                ...
                if count >= self.stuck_detection_threshold:
                    # HUGE penalty for being stuck in loop
                    stagnation_penalty = -2.0 * (count / 10.0)  # Up to -2.0
                    shaped_reward += stagnation_penalty
                    print(f"⚠️ STAGNATION DETECTED! Action {most_common_action} repeated {count}/10 times. Penalty: {stagnation_penalty:.2f}")
        
        if self.step_count > 50 and abs(shaped_reward) < 0.01:
            # Agent is doing nothing useful - penalize based on action
            if action == 11:  # search
                shaped_reward -= 0.1  # Penalty for excessive searching
            else:
                shaped_reward += 0.05  # Small reward for doing ANYTHING else

        # Clip shaped reward to prevent extreme values
        shaped_reward = np.clip(shaped_reward, -5.0, 25.0)
    ...
    def reset(self):
        ...
        # Don't reset episode stats - used for adaptive shaping
    
    def get_episode_stats(self):
        """Get episode statistics for analysis"""
        return {
            'exploration_count': self.episode_exploration_count,
            'unique_positions': len(self.visited_positions),
            'combat_count': self.episode_combat_count,
...
class PrioritizedReplayBuffer(PPOBuffer):
    """Buffer with prioritized experience replay"""
    
    def __init__(self, max_size=4096, alpha=0.6):
        super().__init__(max_size)
        self.alpha = alpha  # Prioritization strength
        self.priorities = []
    
    def add(self, obs, action, reward, value, log_prob, done):
        super().add(obs, action, reward, value, log_prob, done)
        
        # High priority for: large rewards, deaths, level-ups
        priority = abs(reward) + 1.0
        if done and reward < 0:
            priority *= 3.0  # Death is very important
        if reward > 10:
            priority *= 2.0  # Level up is very important
...
class EnhancedNetHackPPOAgent:
    """Fixed version with proper update scheduling"""
    ...
    def __init__(self, action_dim=23, learning_rate=3e-4, gamma=0.99, clip_ratio=0.2):
        ...
        # FIX: Separate learning rates for better stability
        ...
        # FIX: SMALLER buffer for more frequent updates
        self.buffer = PPOBuffer(max_size=512)  # Was 4096, now 512
        ...
        self.last_action = None
        self.updates_per_episode = 0
    
    def update(self, epochs=4, batch_size=64):
        """Improved update with gradient clipping and learning rate schedule"""
        ...
        for epoch in range(epochs):
            ...
            # Actor update with INCREASED entropy bonus
            ...
            # FIX: INCREASED entropy bonus from 0.05 to 0.15 for more exploration
            actor_loss = -torch.min(surr1, surr2).mean() - 0.15 * entropy
        ...
        self.updates_per_episode += 1
        
        # Print update statistics
        if self.updates_per_episode % 10 == 0:
            print(f"  Update {self.updates_per_episode}: "
                  f"Actor Loss: {total_actor_loss/epochs:.4f}, "
                  f"Critic Loss: {total_critic_loss/epochs:.4f}")
    
    def train(self, env, num_episodes=100, update_freq=256):  # REDUCED from 2048 to 256
        ...
        for episode in range(num_episodes):
            ...
            while True:
                ...
                shaped_reward = self.reward_shaper.shape_reward(next_obs, reward, done, info, action)
                ...
                # FIX: Update MUCH more frequently
                if len(self.buffer) >= update_freq:
                    self.update(epochs=4, batch_size=min(64, len(self.buffer)))
                    self.buffer.clear()
            ...
            if episode % 10 == 0:
                ...
                avg_shaped_reward = np.mean(list(self.shaped_rewards)[-10:])
        ...
        try:
            ...
            if self.llm_advisor.should_call_llm():
                try:
                    ...
                    semantic_desc = self.semantic_descriptor.generate_full_description(obs, processed_obs,recent_actions=self.recent_actions)
...
class MonitoredLLMGuidedNetHackAgent(LLMGuidedNetHackAgent):
    ...
    async def train_episode_monitored(agent, env, episode):
        ...
        detailed_logger = DetailedStepMonitor()
        ...
        while True:
            ...
            agent.recent_actions.append(action_name)
            ...
            if len(agent.buffer) >= 64:  # Minimum batch size
                print(f"\n{agent.monitor.CYAN}Updating neural networks after episode...{agent.monitor.ENDC}")
                agent.update(epochs=4, batch_size=min(64, len(agent.buffer)))
                agent.buffer.clear()
...
class ImprovedLLMGuidedPPOActor(nn.Module):
    ...
    def __init__(self, action_dim=23, llm_guidance_weight=0.9, llm_override_threshold=0.6):
        ...
        self.llm_override_threshold = 0.4  # REDUCED from 0.7
        
        # FIX: Add curriculum learning for LLM guidance
        self.episodes_trained = 0
        self.max_guidance_weight = 0.6  # Never go above 0.6
    ...
    def adjust_guidance_weight(self, performance_improvement):
        """Dynamically adjust LLM guidance based on learning progress"""
        self.episodes_trained += 1
        
        # Gradually increase LLM guidance if agent isn't learning
        if performance_improvement < 0.1 and self.episodes_trained > 20:
            self.llm_guidance_weight = min(
                self.llm_guidance_weight + 0.05,
                self.max_guidance_weight
            )
        # Decrease if agent is learning well (let RL take over)
        elif performance_improvement > 0.5:
            self.llm_guidance_weight = max(
                self.llm_guidance_weight - 0.05,
                0.1
            )
        
        print(f"  Adjusted LLM guidance weight to: {self.llm_guidance_weight:.2f}")
    ...
    def forward(self, obs, reset_hidden=False, llm_advice=None):
        ...
        if llm_advice and self.llm_guidance_weight > 0:
            ...
            # FIX: BALANCED guidance - don't overwhelm base policy
            max_guidance = guidance_bias.max().item()
            
            if max_guidance > 0.5:
                # REDUCED from 20x to 5x boost
                guided_logits = base_logits + (guidance_bias * 5.0 * self.llm_guidance_weight)
            else:
                guided_logits = base_logits + (guidance_bias * 2.0 * self.llm_guidance_weight)
...
YOUR TASK: Suggest 3-5 SPECIFIC actions in priority order.

RULES:
1. Use EXACT action names: move_north, move_south, move_east, move_west, kick, search, pickup
2. If monster nearby and health > 60%: suggest "kick" to attack
3. If monster nearby and health < 40%: suggest movement AWAY from monster
4. If no threats: suggest "search" or "pickup" or directional movement
5. VARY directions! Don't always say move_east
6. If stuck (reward very negative): suggest different direction

RESPOND WITH VALID JSON ONLY:
{{
  "action_suggestions": ["move_north", "kick", "search"],
  "immediate_priority": "one clear goal",
  "risk_assessment": "low/medium/high",
  "opportunities": "what rewards are available",
  "strategy": "brief plan"
}}
...
class LearningRateScheduler:
    ...
    def __init__(self, optimizer, initial_lr=3e-4, patience=10, factor=0.5):
    ...
    def step(self, current_reward):
        ...
        if self.episodes_without_improvement >= self.patience:
            ...
            for param_group in self.optimizer.param_groups:
                ...
                new_lr = max(old_lr * self.factor, 1e-6)  # Don't go below 1e-6
                param_group['lr'] = new_lr
                print(f"  Reduced learning rate: {old_lr:.6f} -> {new_lr:.6f}")
            
            self.episodes_without_improvement = 0

class DetailedStepMonitor:
    """
    Print detailed info every step to understand what's happening
    """
    
    @staticmethod
    def print_detailed_step(step, action_name, obs, reward, shaped_reward):
        ...
        if len(stats) > 20:
            ...
            # Only print every 10 steps or when something important happens
            if step % 10 == 0 or abs(reward) > 0.1 or abs(shaped_reward) > 0.5:
                print(f"Step {step:4d} | {action_name:12s} | "
                      f"HP:{hp:2d}/{max_hp:2d} | Lvl:{level} | XP:{xp:3d} | "
                      f"Depth:{depth} | Gold:{gold:3d} | "
                      f"Pos:({pos_x},{pos_y}) | "
                      f"R:{reward:+.3f} | SR:{shaped_reward:+.3f}")
